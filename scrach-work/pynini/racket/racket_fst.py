...
def racket_fst():
    ...
    op = pynini.union(
        ...
        basic_token('op', pynini.closure(pynini.cross(GROUP + delete_space, 'group: "1" '), upper=1) +
                    pynini.cross(OF, f'text: "{OF}"')))

    graph = maybe_delete_space + pynutil.join(pynini.union(op, atom), delete_space) + maybe_delete_space
...
def symbol_fst():
    ...
    graph = pynini.union(*[pynini.cross(words_to_fst(words), pynini.escape(sym))
...
def name_fst(cardinal: CardinalFst, symbol):
    ...
    undelimited = pynutil.add_weight(
        pynini.union(no_delimit,
                     (pynini.closure(no_delimit + delete_space, upper=1) + word +
                      pynini.closure(delete_space + no_delimit +
...
def basic_fst(number, symbol):
    ...
    token = (pynutil.add_weight(number, -0.1) |
...
def string_fst(basic):
    ...
    graph = (pynutil.delete(STRING + delete_space + OF) +
...
def remove_and(cardinal: CardinalFst):
    ...
    graph = (pynini.cdrewrite(pynutil.delete("and"), WHITE_SPACE, WHITE_SPACE, sigma) @ graph)
